/* ============================================================
   ...
   Implements `AiAnalyser` against `claude-sonnet-4-6` via the
   official Anthropic SDK. Pattern matches `StubStripeAdapter` /
   `RuleBasedAiAnalyser` — same interface, swap-in at the
   `ControlPlane` deps boundary, no other call sites move.
...
export interface ClaudeAiAnalyserOpts {
  ...
  apiKey?: string;
  /** Fallback used on any error or when no API key is configured. */
  fallback: AiAnalyser;
}
...
export class ClaudeAiAnalyser implements AiAnalyser {
  ...
  constructor(opts: ClaudeAiAnalyserOpts) {
    ...
    if (!apiKey) {
      this.client = null;
      this.label = "Claude (not configured — using rule-based fallback)";
      this.live = false;
      return;
    }
  ...
  async analyseDeploy(
    ...
    if (!this.client) return this.fallback.analyseDeploy(input);
    try {
      const facts = JSON.stringify(
        {
          ...
          rollbackTarget:
            input.allDeployments
              .filter(
                (d) => d.id !== input.deployment.id && d.status === "live",
              )
              .pop()?.id ?? null,
        },
        null,
  ...
  async analyseCost(input: AnalyseCostInput): Promise<CostRecommendation[]> {
    if (!this.client) return this.fallback.analyseCost(input);
    try {
      const facts = JSON.stringify(
        {
  ...
  private async callTool<T>(tool: Anthropic.Tool, factsJson: string): Promise<T> {
    if (!this.client) throw new Error("no anthropic client");
    const response = await this.client.messages.create({
      model: MODEL,
      max_tokens: MAX_TOKENS,
      // Stable across every call — caching reuses the prefix and keeps
      // the per-tick LLM spend bounded.
      system: [
        {
          type: "text",
          text: SYSTEM_PROMPT,
